		...
		<header className='absolute inset-x-0 top-0 z-50'>
			<nav className='flex items-center justify-between p-6 lg:px-8'>
				<div className='flex lg:flex-1'>
					<a href="/" className='-m-1.5 p-1.5 text-orange-500 text-lg font-extrabold'>
				...
				<div className="hidden lg:flex lg:gap-x-12">
					{
						navigation.map((item) => (
							<a key={item.name} href={item.href} className="text-sm font-semibold leadin-6 text-gray-900">
								{item.name}
							</a>
						))
					}
				...
				<Dialog.Panel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
					...
					<div className="mt-6 flow-root">
						<div className="-my-6 divide-y divide-gray-500/10">
							...
							<div className="py-6">
								<a href="*" className="-mx-3 block  bg-orange-500 rounded-lg px-3 py-2 5 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50">Sign Up</a>
							</div>
						</div>
